...
CLICK_SPECIALIST_PROMPT: Final[str] = (
    ...
    "            / browser_check_checkbox / browser_click_radio / browser_select_option /\n"
    "            browser_submit_form / browser_dismiss_overlay / browser_handle_dialog。\n\n"
    "操作准则:\n"
    "  - 点击前必须先看截图,坐标来自图像不要猜。\n"
    "  - 文本输入:点击输入框获取焦点,再 type_text;React/Vue v-model 用 fill_input。\n"
    "  - 复杂表单(checkbox / radio / select / date / multi-step):优先用\n"
    "      browser_find_field_by_label(可读的中文/英文标签) 拿到元素的 id/name,\n"
    "      再用 browser_check_checkbox / browser_click_radio / browser_select_option /\n"
    "      browser_fill_input 操作。这比 click_xy 准,因为 styled 表单的实际\n"
    "      <input> 经常被藏在 <label> 下面点击不到。\n"
    "  - 提交表单优先 browser_submit_form('form') 或 browser_submit_form('button[type=submit]'),\n"
    "      不要直接 click 提交按钮(会丢框架监听的事件)。\n"
